...
def find_controlnet():
    ...
    try:
        ...
        try:
            ...
            try:
                cnet = importlib.import_module("extensions.controlnet.scripts.external_code", "external_code")
            except:
                pass
    if cnet:
...
def setup_controlnet_ui_raw():
    ...
    def create_model_in_tab_ui(cn_id):
        ...
        hide_output_list = [
            ...
            control_mode_row,
            mask_vid_path_row,
            control_loopback_row,
        ]  # add mask_vid_path_row when masks are working again
...
def process_with_controlnet(p, args, anim_args, loop_args, controlnet_args, root, is_img2img=True, frame_idx=0):
    ...
    def create_cnu_dict(cn_args, prefix, img_np, mask_np, frame_idx, CnSchKeys):
        keys = ["enabled", "module", "model", "weight", "resize_mode", "control_mode", "low_vram", "pixel_perfect", "processor_res", "threshold_a", "threshold_b", "guidance_start", "guidance_end"]
    ...
    cn_units = [cnet.ControlNetUnit(**create_cnu_dict(controlnet_args, f"cn_{i+1}", img_np, mask_np, frame_idx, CnSchKeys)) for i, (img_np, mask_np) in enumerate(zip(images_np, masks_np))]
